...
base_time64_t base_pivot_time_t (const time_t * now, base_time64_t *_t)
{
	base_time64_t t;
	t = *_t;
	if (now && sizeof (time_t) == 4) {
		...
		if (_now < 1106500000 /* Jan 23 2005 - date of writing */ )
			_now = 2147483647;
		if ((base_time64_t) t + ((base_time64_t) 1 << 31) < (base_time64_t) _now)
			t += (base_time64_t) 1 << 32;
	}
	return t;
}

static struct tm *_gmtime64_r (const time_t * now, base_time64_t *_t, struct tm *p)
{
	...
	v_tm_sec = ((base_time64_t) t % (base_time64_t) 60);
	t /= 60;
	v_tm_min = ((base_time64_t) t % (base_time64_t) 60);
	t /= 60;
...
struct tm *base_gmtime64_r (const base_time64_t *_t, struct tm *p)
{
	base_time64_t t;
	t = *_t;
...
static struct tm *_localtime64_r (const time_t * now, base_time64_t *_t, struct tm *p)
{
	base_time64_t tl;
	time_t t;
	...
	{
        //当时间小于1970年1月1日 - (2^31 + 1)秒(即1901年几月)时会出错，不知道为什么要这样做。 by hch
		t = *_t ;
...
struct tm *base_localtime64_r (const base_time64_t *_t, struct tm *p)
{
	base_time64_t tl;
	tl = *_t;
